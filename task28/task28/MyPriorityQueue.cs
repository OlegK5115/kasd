...
namespace task28
{
    ...
    public class MyPriorityQueue<T> : IMyQueue<T>
    {
        ...
        {
            foreach (T element in a.ToArray()) Add(element);
        }
        public void Clear()
        {
            queue = new MyPriotiryTask<T>[11];
        ...
        public void Print()
        {
            foreach (MyPriotiryTask<T> task in queue.Where(value => value != null)) Console.WriteLine("val: {0}  prior: {1}", task.value, task.priority);
        }
    }
}
